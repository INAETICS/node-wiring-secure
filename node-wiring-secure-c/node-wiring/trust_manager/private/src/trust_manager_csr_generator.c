...
#include "trust_manager_csr_generator.h"
...
mbedtls_pk_context* generate_keypair ()
{
    //Init values
    mbedtls_rsa_context* rsa = malloc(sizeof(mbedtls_rsa_context));
    int ret = 0;

    //The key struct to be returned
...
//    mbedtls_ctr_drbg_context ctr_drbg;
    ...
    opt.subject_name        = "CN=Cert,O=mbed TLS,C=UK";
    ...
    ret = mbedtls_pk_write_key_der( key, buffer_priv, sizeof(buffer_priv) );
    if( ret != 0 )
    {
    ...
    unsigned char buffer_pub[4096];
    memset( buffer_pub, 0, 4096 );
    ret = mbedtls_pk_write_pubkey_pem( key, buffer_pub, sizeof(buffer_pub) );
    if( ret != 0 )
    {
        mbedtls_printf( " failed\n  !  mbedtls_pk_write_pubkey_pem returned %d", ret );
        goto exit;
    }
    printf(buffer_pub);
    printf("\n\n*** Public Key ***\n%s\n", buffer_pub);
...
char* generate_csr()
{
    ...
    mbedtls_pk_context* key = generate_keypair();
