...
/**
 * Activator and configuration manager for the Amdatu HTTP Wiring Admin service
 * implementation.
 * <p>
 * Configuration can be provided through cm as well as system properties. The
 * former take precedence and in addition some fallbacks and defaults are
 * provided. See {@link HttpsAdminConstants} for supported configuration
 * properties.
 * <p>
 * Note that any effective configuration change will close all existing import-
...
public final class Activator extends DependencyActivatorBase implements ManagedService, HttpsAdminConfiguration {
	...
	public void start(BundleContext context) throws ServletException {
	    ...
	    Hashtable<String, Comparable> props = new Hashtable<>();
	    props.put("pattern", "/.*");
	    props.put("init.message", "filter!");
	    props.put("service.ranking", "1");

	    context.registerService(Filter.class.getName(), new ClientCertificateEnforcementFilter(), props);
	}
	...
	public void updated(Dictionary<String, ?> properties) throws ConfigurationException {
		...
		// first parse timeout to local variables, in order to make this method
		// "transactional"
		// assign values to fields after baseUrl was successfully
		int connectTimeout = getConfigIntValue(m_context, CONNECT_TIMEOUT_CONFIG_KEY, properties,
	...
	private void registerConfigurationService() {
		...
		Component component = createComponent().setInterface(ManagedService.class.getName(), properties)
				.setImplementation(this).setAutoConfig(DependencyManager.class, false)
